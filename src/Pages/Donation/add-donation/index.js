...
const AddDonation = ({ onSuccess }) => {
    ...
    const fetchDonationTypes = async () => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/donationDetail/getAllDonationTypes`
        );
  ...
  const handlePaymentChange = (index, e) => {
    ...
    const totalAmount = newPayments.reduce(
  ...
  const handleYesNoChange = (value) => {
    ...
    setShowInput(value === true); // Show input box only if "Yes" is selected
  };

  const [pendingAmount, setPendingAmount] = useState(""); // Default language is English
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/donationDetail/createDonation`,
        {
          method: "POST",
          headers: {
  ...
  const headings = {
    ...
    pendingMoney: isHindi ? "बकाया राशि" : "Pending Amount"
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-2 p-4 bg-white ">
        <div className="grid md:grid-cols-3 gap-4">
          ...
          {showInput && (
            <>
              <label>
                {headings.pendingMoney}
                <input
                  type="text"
                  placeholder="Enter Pending Amount"
                  className="input mt-1 border border-gray-300 rounded p-2 w-full"
                  onChange={(e) =>
                    handlePendingAmountChange(e)
                  }
                />
              </label>
            </>
          ...
          {paymentType === "cheque" && (
            <>
              ...
              <table className="w-full border-collapse border border-gray-300 mt-2">
                <thead>
                  <tr>
                    <th className="border border-gray-300 p-2 flex justify-between items-center text-nowrap">
                      {headings.typeDonaion}
                      <span onClick={handleAddPayment}>
                        <FaPlusSquare className="text-blue-500 text-3xl" />
                      </span>
