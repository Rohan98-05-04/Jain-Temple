...
export default function AddOutsiderDonation() {
  ...
  const [selectedAmounts, setSelectedAmounts] = useState({}); // Store selected donation amounts
  ...
  const addEvent = () => {
    ...
    const inputData = {
        firstName,
        lastName,
        phoneNumbers:[{Phonenumber1:phoneNumberOne}],
        dob,
        donation:{
      eventId,
      eventCategory,
...
console.log('guestform',inputData)
    // Log the input values object
    setIsLoading(true);
    
    const fetchData = async () => {
      const response = await fetch(
        `${API_BASE_URL}/donor/registerGuest`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
