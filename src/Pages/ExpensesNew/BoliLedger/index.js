...
const BoliLedger = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const response = await fetch(`${API_BASE_URL}/expenses/getAllExpenses?expenseType=expenseledger&page=${activePage}&size=${size}&search=${search}`, {
        method: 'GET',
        headers: {
  ...
  const handleDelete = async () => {
    ...
    setIsLoading(true);
    const response = await fetch(`${API_BASE_URL}/expenses/deleteExpense/${deleteId}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${parseToken}`,
  ...
  return (
    <div className="donarPage">
      ...
      <div className="donarDetailMainPage">
        <Section>
          <h2 className="text-2xl font-bold mb-4">Expense Ledger</h2>
          <div className="flex flex-col md:flex-row justify-between mb-3">
            ...
            {/* Add Donar Button */}
            <div>
              <button onClick={openModal} className="bg-orange-400 text-white py-2 px-4 rounded-md">Add Expense Ledger</button>
            </div>
          </div>
        </Section>
