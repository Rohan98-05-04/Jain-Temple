...
const EditBoliVoucher = ({ id }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        const response = await fetch(`${API_BASE_URL}/expenses/updateExpense/${id}`, {
            method: 'PUT',
            headers: {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const response = await fetch(`${API_BASE_URL}/expenses/getExpenseById/${id}`);
            if (response.ok) {
                const data = await response.json();
                setFormData(data.data);
                setLedgerNumber(data.data.ledgerNo);
