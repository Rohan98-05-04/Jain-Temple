...
import Modal from './add-boli/ModalAddBoliHead';
...
const BoliHead = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/boliDetail/getAllBoliHeads?page=${activePage}&size=${size}&search=${search}`, {
          method: 'GET',
          headers: {
  ...
  return (
    <div className="">
      ...
      <div className="">
        <Section>
          <div className="mb-4">
            <h2 className="text-2xl font-bold">Boli Heads</h2>
          </div>
          <div className="flex flex-col md:flex-row justify-between mb-3">
            ...
            <div>
              <button onClick={openModal} className="bg-orange-400 text-white py-2 px-4 rounded-md">Add Boli Head</button>
            </div>
          </div>
        </Section>
        <Section>
