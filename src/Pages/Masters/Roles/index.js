...
const Role = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/role/getAllMemberRoles?page=${activePage}&size=${size}&search=${search}`, {
          method: 'GET',
          headers: {
        ...
        setData(result.data);
        setDonorData(result.data.roles);
      } catch (error) {
      ...
      <div className="bg-white">
        <Section>
          ...
          <div className="flex flex-col md:flex-row justify-between mb-3">
            ...
            <div>
              <button onClick={openModal} className="bg-orange-400 text-white py-2 px-4 rounded-md">Add Role</button>
            </div>
          </div>
        </Section>
        <Section>
