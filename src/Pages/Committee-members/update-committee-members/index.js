...
export default function UpdateCommitteeMembers() {
    ...
    const validateEmail = (email) => {
      ...
      const handleRoleChange = (selectedOption) => {
        ...
        if (selectedOption) {
          ...
          const role = ExistingRole.find((role) => role._id === selectedOption.value);
          setRole(role._id)
    ...
    const fetchData = async () => {
    const response = await fetch(`${API_BASE_URL}/users/getUser/${slug.slug}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${parseToken}`,
    ...
    const data = await response.json();
     ...
     if(jsonString){
       ...
       setPhonenumber(existingObject.phonenumber || null);
       setLine_1(existingObject.address?.line_1 || '');
       setLine_2(existingObject.address?.line_2 || '');
       setLine_3(existingObject.address?.line_3 || '');
       ...
       setRoleOption({
        value: existingObject.role?._id,
        label: existingObject.role?.name
      });
    ...
    const handleSignup = () => {
      ...
      const fetchData = async () => {
        const response = await fetch(`${API_BASE_URL}/users/updateUser/${slug.slug}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${parseToken}`,
          },
          body: JSON.stringify({name, phonenumber,role,address:{line_1,
            line_2,pincode,line_3,
            city,
            state,
            country},loginId, email }),
        });
