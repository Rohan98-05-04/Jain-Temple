...
const BoliGroup = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const response = await fetch(`${API_BASE_URL}/boliDetail/getAllBolis?boliType=${"boligroup"}&page=${activePage}&size=${size}&search=${search}`, {
        method: 'GET',
        headers: {
  ...
  return (
    <div className="donarPage">
      ...
      <div className="donarDetailMainPage">
        <Section>
          <h2 className="text-2xl font-bold mb-4">Boli Group</h2>
          <div className="flex flex-col md:flex-row justify-between mb-3">
            ...
            {/* Add Donar Button */}
            <div>
              <button onClick={openModal} className="bg-orange-400 text-white py-2 px-4 rounded-md">Add Boli Group</button>
            </div>
          </div>
        </Section>
