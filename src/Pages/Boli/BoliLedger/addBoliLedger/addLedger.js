...
const AddBoliLedger = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                toast.success('Boli Ledger created successfully');
                router.push('/boli#ledger');
                fetchBoliHeads();
            } else {
                const errorData = await response.json();
                toast.error(`Failed to create Boli: ${errorData.message || 'Unknown error'}`);
            }
        } catch (error) {
            toast.error('Failed to create Boli: Network error');
            console.error("Error creating Boli:", error);
        }
    };
