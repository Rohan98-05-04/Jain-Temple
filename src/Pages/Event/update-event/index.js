...
const response = await fetch(`${API_BASE_URL}/event/getevent/${slug.slug}`, {
method: 'GET',
headers: {
...
const data = await response.json();
 ...
 setEventCategory(existingObject.eventCategory || '');
 let selectedOption = existingObject.eventCategory.map((item) => {
...
fetchData();
  ...
  const addEvent = () => {
    ...
    const fetchData = async () => {
      const response = await fetch(`${API_BASE_URL}/event/updateevent/${slug.slug}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${parseToken}`,
