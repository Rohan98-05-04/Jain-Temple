...
export default function AddMandirUsers() {
    ...
    const [isHead, setIsHeadOfFamily] = useState(true);
  ...
  //   useEffect(() => {
  //     const jsonString = localStorage.getItem('donarDetail');
  //   if(jsonString){
  //     const existingObject = JSON.parse(jsonString) || {};
  ...
  // setPhoneNumberOne(existingObject.phoneNumberOne || '');
  // setPhoneNumberTwo(existingObject.phoneNumberTwo || '');
  ...
  // setAddressLineOne(existingObject.addressLineOne || '');
  // setAddressLineTwo(existingObject.addressLineTwo || '');
  // setAddressLineThree(existingObject.addressLineThree || '');
  // setCity(existingObject.city || '');
  // setState(existingObject.state || '');
  // setPincode(existingObject.pincode || '');
  // setCountry(existingObject.country || '');
  // setMember(existingObject.members || []);
  //   }
  //   }, []);
    ...
    const handleSignup = () => {
        ...
        if (!areAllMembersValid) {
          toast.error('Please fill required fields for all members.', {
            position: toast.POSITION.TOP_RIGHT,
          });          return;
        }
       ...
       let inputData ={
        image,firstName,lastName,phoneNumbers:[{Phonenumber1:phoneNumberOne,Phonenumber2:phoneNumberTwo}],gotr,dateOfAnniversary,dob,
              occupation,isHead, email,aadharNo,aadharCardImage:aadharcardimage,bloodGroup,address:{line_1:addressLineOne
                ,line_2:addressLineTwo,line_3:addressLineThree,
                city,state,pincode,country:"India"},members
       }
       setIsLoading(true);
       
       const fetchData = async () => {
         const response = await fetch(`${API_BASE_URL}/donor/donarRegister`, {
           method: 'POST',
           headers: {
        ...
        <div className="donarPage">
         <ToastContainer position="top-right" autoClose={5000} />
          <div className="addDonarForm">
            <h2 className={`${styles.formHeaderext}`}>Enter donar&apos;s details</h2>
            <form>
