...
export default function UpdateMandirUsers() {
...
fetchData();
    ...
    const handleSignup = () => {
        ...
        if(!isHead){
          ...
          const fetchData = async () => {
            const response = await fetch(`${API_BASE_URL}/donor/donarupdate/${slug.slug}`, {
              method: 'PUT',
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${parseToken}`,
              ...
              body: JSON.stringify({image,firstName,lastName, "phoneNumbers": [
                {
                  "Phonenumber1": phoneNumberOne,
                  "Phonenumber2": phoneNumberTwo
                }
              ],dob,gotr,dateOfAnniversary,isHead,
